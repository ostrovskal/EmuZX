...
void DecoderZX::ops01_CB() {
	...
	byte ret = (((prefix ? *preg : *fromRON1(typeOps)) & (1 << ops)) ? 1 : 0);
	byte fz = !ret;
	byte fs = ((ops == 7 && ret) ? 128 : 0);
	byte f5 = (typeOps == 6 ? 0 : ((ops == 5 && ret) << 5));
	byte f3 = (typeOps == 6 ? 0 : ((ops == 4 && ret) << 3));
