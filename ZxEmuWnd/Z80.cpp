...
bool saveZ80(const StringZX& path) {
	...
	try {
		...
		if(h) {
			...
			memcpy(&z80.BC_, &regsZX[RC + 14], 8);
			z80.STATE1 = ((_R & 128) >> 7) | ((_PORT_FE & 7) << 1) | 32;
			if(_write(h, &z80, sizeof(HEAD1_Z80)) != sizeof(HEAD1_Z80)) throw(0);
