...
static ssh_w volTable[zxSound::countRegAY] = {
...
void zxSound::play(char* buffer, int len) {
...
void zxSound::setTicksPerFrame() {
	double koef = (double)timeout / (double)TIMER_CONST;
...
void zxSound::writeNVRegAY(int reg, int val) {
...
void zxSound::writeRegAY(int reg, int val) {
	if(reg < 16) {
		curRegsAY[reg] = val;
...
void zxSound::applyRegisterAY(int reg, int val) {
...
void zxSound::update() {
	if(PSGcounter > 0) {
		...
		for(int i = 0; i < PSGcounter; i++) PSGARRAY[i].pos = (SND_TICKS_PER_FRAME[0] - 1) * PSGARRAY[i].tcounter / tcounter;
	}
	// Генерим звук
	int j = 0;
...
void zxSound::mix(ssh_w* buf, int num, int offs, int count, ...) {
	va_list ap;
	int i, j;

	va_start(ap, count);

	for(i = 0; i < count; i++) mixing_ch[i] = va_arg(ap, ssh_w*);

	for(i = 0; i < SND_TICKS_PER_FRAME[num]; i++) {
		buf[i * 2 + offs] = 0;
		for(j = 0; j < count; j++) buf[i * 2 + offs] += mixing_ch[j][i << num] / count;
	}
}

void zxSound::AY_CHANNEL::gen(int i, zxSound* snd) {
