...
protected:
	...
	inline ssh_w* fromRP_SP(ssh_b rp) { return (ssh_w*)&regsZX[cnvPrefSP[prefix + (rp & 6)]]; }
	
	// преобразуем 2 битный номер регистровой пары в указатель на регистр(с учетом AF)
	inline ssh_w* fromRP_AF(ssh_b rp) { return (ssh_w*)&regsZX[cnvPrefAF[prefix + (rp & 6)]]; }

	// преобразуем 3 битный номер половинки регистра в указатель на регистр
	inline ssh_b* fromRON(ssh_b ron) {
		if(ron == 6) return (preg = &memZX[(*fromRP_SP(RL) + (char)(prefix ? read_mem8PC() : 0) & 0xffff)]);
		return &regsZX[cnvPrefRON[prefix + ron]];
	}

	// преобразуем 3 битный номер половинки регистра в указатель на регистр
	inline ssh_b* fromRON1(ssh_b ron) {
		if(ron == 6) return &memZX[*_HL];
		return &regsZX[cnvPrefRON[ron]];
	}
